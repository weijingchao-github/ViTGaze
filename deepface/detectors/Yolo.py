...
class YoloClient(Detector):
    ...
    def build_model(self) -> Any:
        ...
        if not os.path.isfile(weight_path):
            logger.info(
                f"Downloading Yolo weights from {WEIGHT_URL} to {weight_path}..."
            )
            try:
            ...
            logger.info(
                f"Yolo model is just downloaded to {os.path.basename(weight_path)}"
            )
    ...
    def detect_faces(self, img: np.ndarray):
        ...
        # Detect faces
        results = self.model.predict(img, verbose=False, show=False, conf=0.5)[0]
        ...
        for result in results:
            ...
            detect_result_one_piece = {
                "xywh": (int(x), int(y), int(w), int(h)),
                "confidence": confidence,
                "left_eye": left_eye,
                "right_eye": right_eye,
            }
            resp.append(detect_result_one_piece)
